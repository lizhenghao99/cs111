...
void process_command(char* command)
{
	...
	else if (strcmp(command, "OFF\n") == 0)
	{
		...
		char offbuf[64];
		sprintf(offbuf, "%02d:%02d:%02d SHUTDOWN\n", local->tm_hour, local->tm_min, local->tm_sec);
		SSL_write(client, offbuf, strlen(offbuf));
	...
	while (1)
	{
		...
		if (flag_start && curr-prev >= period)
		{
			...
			if (flag_log)
				dprintf(fd, "%02d:%02d:%02d ", local->tm_hour, local->tm_min, local->tm_sec);
			char tempbuf[64];
			sprintf(tempbuf, "%02d:%02d:%02d %.1f\n", local->tm_hour, local->tm_min, local->tm_sec, temp);
			SSL_write(client, tempbuf, strlen(tempbuf));
			if (flag_log)
				dprintf(fd, "%.1f\n", temp);
			prev = curr;
		}
...
{		
	...
	client = SSL_new(context);
	if (client == NULL)
	{
		fprintf(stderr, "ERROR: failed to create SSL client\n");
		exit(2);
	}

	int setfdrc = SSL_set_fd(client, sockfd);
	if (setfdrc == 0)
	{
		fprintf(stderr, "ERROR: failed to set SSL client to socketfd\n");
		exit(2);
	}

	int connectrc = SSL_connect(client);
	if (connectrc != 1)
	{
		fprintf(stderr, "ERROR: failed to connect SSL\n");
		exit(2);
	}

	char idbuf[32];
	sprintf(idbuf, "ID=%s\n", id);
	int writerc = SSL_write(client, idbuf, strlen(idbuf));
	if (writerc <= 0)
	{
		fprintf(stderr, "ERROR: failed to write through SSL\n");
		exit(2);
	}
	dprintf(fd, "ID=%s\n", id);
